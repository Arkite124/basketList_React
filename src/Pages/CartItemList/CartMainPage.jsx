...
import {DeleteCartOne, GetCart} from "./cartUtil/CartUtil.js";
    ...
    if(loginUser) return(
       ...
       <span onClick={closeModal}><button className="w-[40%] md:w-[7.5rem] h-[3rem] mx-2 rounded-lg bg-red-200 text-black text-xl font-mono hover:bg-red-500 hover:text-white"
            ...
            <button className="w-[40%] md:w-[12.5rem] h-[3rem] mr-3 rounded-lg bg-red-400/50 text-black text-xl font-mono hover:bg-red-400 hover:text-white"
                    ...
                    const cartItemId = list.cartItemId;
                    const cartQty = cartQuantity[cartItemId] ?? list.quantity;
                    ...
                    return (<tr className="h-[25%] md:h-[7.5rem] w-full flex flex-row justify-center items-center flex-pre-wrap my-2" key={list.cartItemId}>
                        <td className="w-[30%] h-full md:w-[10%] m-1 mt-1">
                            <img src={list.products.productImgUrl} alt={"상품사진"} className="w-full h-[40%] md:h-full m-1 mt-2"/></td>
                        <td className={categoryClassText[list.products.productCategory]}>{list.products.productName}</td>
                        <td className={categoryClassText[list.products.productCategory]}>&#8361;{FormatPrice}</td>
                        <td className={categoryClassText[list.products.productCategory]}>
                        ...
                        <td className={categoryClassText[list.products.productCategory]}>&#8361;{FormatSelectedPrice}</td>
                        <td className={categoryClassText[list.products.productCategory]}>{list.addedAt}</td>
                    ...
                    flex justify-center hover:border-transparent hover:bg-rose-700 hover:text-pink-300" type="button" onClick={DeleteCartItem}>삭제하기</button></td>
                    </tr>)})}
                </tbody>
            </table>
        </div>)}